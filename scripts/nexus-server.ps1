...
param(
    ...
    [string]$ComposeProjectName = 'nexus',
...
function Assert-ComposeProjectName {
    param([string]$Value)
    if ([string]::IsNullOrWhiteSpace($Value) -or $Value -notmatch '^[a-z0-9][a-z0-9_-]*$') {
        throw 'ComposeProjectName must start with a lowercase letter or digit and contain only lowercase letters, digits, hyphens or underscores.'
    }
}
...
function New-EnvironmentFile {
    ...
    Assert-ComposeProjectName $ComposeProjectName
...
function Assert-EnvironmentFile {
    ...
    if ($values.ContainsKey('COMPOSE_PROJECT_NAME')) {
        Assert-ComposeProjectName $values['COMPOSE_PROJECT_NAME']
    }
    $required = @(
...
New server (run after git clone):
  powershell.exe -ExecutionPolicy Bypass -File .\scripts\nexus-server.ps1 -Action Initialize `
    ...
    -AiGatewayUrl http://25.31.233.158:8090 -ComposeProjectName nexus_server_fresh
