...
export function SharedStoryPanel({ server, members, currentUser, onClose }: SharedStoryPanelProps) {
  ...
  async function createStory() {
    if (players.length < 2) { setError("Ortak Hikâye en az iki oyuncu gerektirir."); return; }
    setBusy(true); setError(null);
    try { setStory(await coreApi.createSharedStory(server.id, { player_ids: players, ai_players: aiPlayers, theme, length, use_party_lore: useLore }, createKey.current)); }
  ...
  return <div className="settings-panel-backdrop" onClick={onClose}>
    <section className="settings-panel settings-panel--wide story-panel" onClick={(event) => event.stopPropagation()} aria-label="Ortak Hikâye">
      ...
      {!loading && !story && safety ? <div className="story-panel__setup">
        ...
        <section className="story-panel__card"><span>2. OTURUM</span><h3>İki veya üç yolcu seç</h3><div className="story-panel__members">{members.map((member) => <label key={member.id} className={players.includes(member.id) ? "is-selected" : ""}><input type="checkbox" checked={players.includes(member.id)} disabled={member.id === currentUser.id} onChange={() => togglePlayer(member.id)}/>{member.display_name || member.username}</label>)}</div>{players.length === 2 ? <label className="story-panel__ai-toggle"><input type="checkbox" checked={withAi} onChange={(event) => setWithAi(event.target.checked)}/>Üçüncü yolcuyu AI (Yankı) oynasın<small>{withAi ? "Yankı kendi sahnesini oynar ve ortak kararlarda oy kullanır." : "İki yolcuyla bölüm başına iki sahne olur; final eşikleri buna göre ayarlanır."}</small></label> : null}<label>Tema<select value={theme} onChange={(event) => setTheme(event.target.value as typeof theme)}><option value="FANTASY">Fantastik</option><option value="MYSTERY">Gizem</option><option value="SURVIVAL">Hayatta kalma</option></select></label><label>Uzunluk<select value={length} onChange={(event) => setLength(event.target.value as typeof length)}><option value="SHORT">Kısa · 2 bölüm</option><option value="STANDARD">Standart · 3 bölüm</option><option value="LONG">Uzun · 4 bölüm</option></select></label><label><input type="checkbox" checked={useLore} onChange={(event) => setUseLore(event.target.checked)}/>İzinli Party Lore'u yalnız dekoratif kurmaca esintisi olarak kullan</label><button type="button" disabled={busy || players.length < 2} onClick={() => void createStory()}>Hikâyeyi başlat ({players.length + aiPlayers} koltuk)</button></section>
      </div> : null}
      ...
      {story ? <div className="story-panel__story">
        ...
        <section className="story-panel__characters">{story.characters.map((character) => <article key={character.seat} className={story.active_seat === character.seat ? "is-active" : ""}><span>{character.archetype}</span><strong>{character.name}</strong><p>{character.traits.join(" · ")}</p><small>{character.location} · {character.inventory.join(", ")}</small></article>)}</section>
