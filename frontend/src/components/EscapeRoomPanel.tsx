...
export function EscapeRoomPanel({ server, members, currentUser, onClose }: EscapeRoomPanelProps) {
  ...
  // Üç mod: 3 insan · 2 insan + AI üçüncü konsol · 2 insan (iki rollü bulmaca seti).
  const aiPlayers = players.length === 2 && withAi ? 1 : 0;

  async function createRoom() {
    if (players.length < 2) { setError("NADİR-3 en az iki oyuncu gerektirir."); return; }
    setBusy(true); setError(null);
    try { setRoom(await coreApi.createEscapeRoom(server.id, { player_ids: players, ai_players: aiPlayers, timer_mode: timerMode, use_party_lore: useLore }, createKey.current)); }
  ...
  return <div className="settings-panel-backdrop" onClick={onClose}>
    <section className="settings-panel settings-panel--wide escape-panel" onClick={(event) => event.stopPropagation()} aria-label="NADİR-3 Escape Room">
      ...
      {!loading && !room ? <section className="escape-panel__lobby"><h3>Tahliye ekibini oluştur</h3><p>Üç koltukta roller Mühendis, Analist ve Navigatör olur. İki kişilik odada Analist konsolu yoktur; bulmacalar iki role göre yeniden yazılmış settir. Her oyuncu özel konsolunu yalnız kendi hesabından görür.</p><div>{members.map((member) => <label key={member.id} className={players.includes(member.id) ? "is-selected" : ""}><input type="checkbox" checked={players.includes(member.id)} disabled={member.id === currentUser.id} onChange={() => togglePlayer(member.id)}/>{member.display_name || member.username}</label>)}</div>{players.length === 2 ? <label className="escape-panel__ai-toggle"><input type="checkbox" checked={withAi} onChange={(event) => setWithAi(event.target.checked)}/>Üçüncü konsolu AI tutsun<small>{withAi ? "NADİR-3 bulmacaları aynen kalır; AI kendi özel ipuçlarını paylaşır ve senkronda kendi kodunu girer." : "İki konsollu set kullanılır; her zorunlu bulmaca yine iki konsolun birlikte okunmasını ister."}</small></label> : null}<label>Sayaç modu<select value={timerMode} onChange={(event) => setTimerMode(event.target.value as typeof timerMode)}><option value="RELAXED">Rahat · sayaç yok</option><option value="STANDARD_45">Standart · 45 dk</option><option value="CHALLENGE_30">Meydan okuma · 30 dk</option></select></label><label><input type="checkbox" checked={useLore} onChange={(event) => setUseLore(event.target.checked)}/>İzinli Party Lore'u yalnız atmosferde kullan</label><button type="button" disabled={busy || players.length < 2} onClick={() => void createRoom()}>Hava kilidini aç ({players.length + aiPlayers} konsol)</button></section> : null}
      ...
      {room ? <div className="escape-panel__room">
        ...
        <section className="escape-panel__players">{room.players.map((player) => <article key={player.seat} className={player.ai ? "is-ai" : ""}><strong>{player.display_name}</strong><span>{ROLE_LABELS[player.role]}</span></article>)}</section>
        {room.ai_consoles.length ? <section className="escape-panel__ai-console"><h3>AI konsolu paylaşıyor</h3>{room.ai_consoles.map((console) => <article key={console.role}><strong>{ROLE_LABELS[console.role]}</strong>{Object.entries(console.clues).length ? Object.entries(console.clues).map(([nodeId, clue]) => <p key={nodeId}><b>{nodeId}</b> {clue}</p>) : <p>Açık düğüm için paylaşacak veri yok.</p>}</article>)}</section> : null}
        <div className="escape-panel__workspace">
          <section className="escape-panel__graph">{room.nodes.map((node) => <button key={node.id} type="button" disabled={node.status === "LOCKED"} className={`${node.status === "SOLVED" ? "is-solved" : node.status === "AVAILABLE" ? "is-available" : "is-locked"} ${selectedNodeId === node.id ? "is-selected" : ""}`} onClick={() => setSelectedNodeId(node.id)}><span>{node.id} · {node.kind}</span><strong>{node.title}</strong><small>{node.status}{node.kind === "META" ? ` · ${node.submitted_count}/${room.seat_count}` : ""}</small></button>)}</section>
